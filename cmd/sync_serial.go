...
// sync-serial: 按视频顺序逐个下载完再上传（下载→上传为一个原子单元）
var syncSerialCmd = &cobra.Command{
	Use:   "sync-serial",
...
func init() {
	syncSerialCmd.Flags().StringVar(&serialChannelURL, "channel", "", "要顺序同步的频道URL")
	syncSerialCmd.Flags().BoolVar(&serialAll, "all", false, "顺序同步配置中所有频道")
	rootCmd.AddCommand(syncSerialCmd)
}
